...
​
​1
​​ )∗(f(x+1)+f(x+2)+...+f(x+W)) This is because there is a \frac{1}{W}
​W
​
​1
...
Note that we can reduce the time complexity to O(\max(K, W))O(max(K,W)) and the space complexity to O(W)O(W) by only keeping track of the last WW values of dp, but it isn't required.
	 * @return
	 */
