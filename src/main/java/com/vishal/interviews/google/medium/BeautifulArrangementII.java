...
To enumerate each permutation without using library functions, we use a recursive algorithm, where permute is responsible for permuting the indexes of \text{nums}nums in the interval \text{[start, nums.length)}[start, nums.length).
...
Space Complexity: O(n)O(n). We use \text{seen}seen to store whether we've seen the differences, and each generated permutation has a length equal to \text{n}n.
  ...
  public int[] constructArrayNaive(int n, int k) {
      int[] nums = new int[n];
      for (int i = 0; i < n; i++) {
          nums[i] = i+1;
      }
...
Space Complexity: O(n)O(n). Our answer has a length equal to \text{n}n.
   */
  public int[] constructArrayBest(int n, int k) {
     int[] ans = new int[n];
     int c = 0;
     for (int v = 1; v < n-k; v++) {
         ans[c++] = v;
