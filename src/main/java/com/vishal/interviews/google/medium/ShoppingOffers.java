...
If the current offer could be applied, find the minimum cost out of resres and offer\current + shopping(price,special,clone). Here, offer\current refers to the price that needs to be paid for the current offer. Update the resres with the minimum value.
...
We need to note that the cloneclone needs to be updated afresh from needsneeds(coming to the current function call) when we choose a new offer. This needs to be done, because solely applying the next offer could result in a lesser cost than the one resulting by using the previous offer first.
	 ...
	 */
	public int shoppingOffersUsingRecursion(List < Integer > price, List < List < Integer >> special, List < Integer > needs) {
...
In the last approach, we can observe that the same needsneeds can be reached by applying the offers in various orders. e.g. We can choose the first offer followed by the second offer or vice-versa. But, both lead to the same requirement of updated needsneeds and the cost as well. Thus, instead of repeating the whole process for the same needsneeds state through various recursive paths, we can create an entry corresponding to the current set of needsneeds in a HashMap, mapmap, which stores the minimum cost corresponding to this set of needsneeds. Thus, whenever the same call is made again in the future through a different path, we need not repeat the whole process over, and we can directly return the result stored in the mapmap.

   */
  public int shoppingOffersRecursionWithMem(List < Integer > price, List < List < Integer >> special, List < Integer > needs) {
 ...
 public int shopping(List < Integer > price, List < List < Integer >> special, List < Integer > needs, Map < List < Integer > , Integer > map) {
     ...
     int j = 0, res = dotRecurionWithMem(needs, price);
     for (List < Integer > s: special) {
         ArrayList < Integer > clone = new ArrayList < > (needs);
         for (j = 0; j < needs.size(); j++) {
             int diff = clone.get(j) - s.get(j);
 ...
 public int dotRecurionWithMem(List < Integer > a, List < Integer > b) {
     int sum = 0;
     for (int i = 0; i < a.size(); i++) {
         sum += a.get(i) * b.get(i);
     }
