 ...
 * https://discuss.leetcode.com/topic/77471/easy-understanding-java-bfs-solution
 * 
 ...
 * 
 * We need to use PriorityQueue instead of standard queue, and record the
 * minimal length of each point.
 *
 */
class SolutionEasy {
	class Point {
	...
	public int shortestDistance(int[][] maze, int[] start, int[] destination) {
		...
		PriorityQueue<Point> list = new PriorityQueue<>((o1, o2) -> o1.l - o2.l); // using
																										  // priority
																										  // queue
...
class SolutionMazeIIBFS {
	...
	public int shortestDistance(int[][] maze, int[] start, int[] destination) {
		...
		if (desL && desR && desD && desU)
			return -1; // all neighbors are walls
		else if (!(desL || desR || desD || desU))
			return -1; // all neighbors are empty spaces
		else if (desL && desR && !desU && !desD)
			return -1; // two opposite neigbhors are walls, and the other two are
						  // empty spaces
		else if (!desL && !desR && desU && desD)
			return -1;// two opposite neigbhors are walls, and the other two are
						 // empty spaces
		...
		/*
		 * BFS; Optimization: use PriorityQueue based on the steps instead of
		 * Queue
		 */
		...
		while (!pq.isEmpty()) {
			 ...
			 * done
			 */
			if (pos.r == destination[0] && pos.c == destination[1])
				return pos.steps;
			for (int[] dir : dirs) {
		...
		return minSteps[destination[0]][destination[1]] == Integer.MAX_VALUE ? -1
				: minSteps[destination[0]][destination[1]];
	}
