		...
		for (int l = 0; l < s.length(); l++) {
			for (int i = 0; i < s.length() - l; i++) {
				...
				if (j - i < 4) {
					...
					dp[i][j] = substr;
					// Loop for trying all results that we get after dividing the
					// strings into 2 and combine the results of 2 substrings
					for (int k = i; k < j; k++) {
					...
					// Loop for checking if string can itself found some pattern in
					// it which could be repeated.
