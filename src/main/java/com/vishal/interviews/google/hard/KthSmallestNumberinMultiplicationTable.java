...
Time Complexity: O(k * m \log m) = O(m^2 n \log m)O(k∗mlogm)=O(m
​2
​​ nlogm). Our initial heapify operation is O(m)O(m). Afterwards, each pop and push is O(m \log m)O(mlogm), and our outer loop is O(k) = O(m*n)O(k)=O(m∗n)
...
Then (for our answer \text{A}A), whenever x ≥ A, enough(x) is True; and whenever x < A, enough(x) is False.
...
​th
​​  row looks like \text{[i, 2*i, 3*i, ..., n*i]}[i, 2*i, 3*i, ..., n*i]. The largest possible k*i ≤ x that could appear is \text{k = x // i}k = x // i. However, if \text{x}x is really big, then perhaps k > n, so in total there are \text{min(k, n) = min(x // i, n)}min(k, n) = min(x // i, n) values in that row that are less than or equal to \text{x}x.
